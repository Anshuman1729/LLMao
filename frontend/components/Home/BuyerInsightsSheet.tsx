  ...
  return (
    <div className="flex items-center gap-6 mt-3">
      <svg width="90" height="90" viewBox="0 0 100 100">
        ...
        />
        {/* First-time arc overlaid on the remaining portion */}
        <circle
          cx="50" cy="50" r={r}
          fill="none"
          stroke="#FFB3D1"
    ...
    <>
      ...
      >
        ...
        <div className="px-5 pb-10">
          ...
          <div className="flex items-center gap-3 bg-pink-50 rounded-2xl p-4 mt-4 cursor-pointer">
          ...
          {/* ── PRODUCT SUGGESTIONS ── */}
          <SectionDivider label="Product Suggestions" />

          <div className="flex gap-3">
            {insights.product_suggestions.map((p, i) => (
              <div key={i} className="flex-1 bg-gray-50 rounded-2xl p-3 flex flex-col items-center gap-1 text-center">
                <span className="text-2xl">{p.emoji}</span>
                <p className="text-xs font-semibold text-gray-800 leading-tight">{p.name}</p>
                <p className="text-[10px] text-gray-500">{p.price_label}</p>
              </div>
            ))}
          </div>
